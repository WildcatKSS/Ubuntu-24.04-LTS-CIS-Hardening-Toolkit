...
Bash-scripts voor het hardenen van een schone Ubuntu 24.04 LTS Server installatie op basis van de **CIS Ubuntu Linux 24.04 LTS Benchmark v1.0.0**. Het hoofdscript vraagt tijdens het uitvoeren welk CIS-profiel (Level 1 of Level 2 Server) je wilt toepassen.

> ⚠️ **Disclaimer:** Deze toolkit wijzigt diepgaand systeemconfiguratie. Gebruik uitsluitend op verse installaties of in een testomgeving voordat je hem op productie loslaat. Maak altijd een snapshot/backup. De auteur is niet verantwoordelijk voor verlies van toegang of functionaliteit.

-----

## 📋 Projectdoel

Een schone installatie van Ubuntu Server 24.04 LTS in lijn brengen met de aanbevelingen van de CIS Benchmark, zonder afhankelijk te zijn van commerciële tooling (zoals Ubuntu Pro / USG). De toolkit is modulair, idempotent en leesbaar, zodat iedere wijziging traceerbaar en omkeerbaar is.

### Waarom bash en niet Ansible?

- Nul externe afhankelijkheden buiten een standaard Ubuntu installatie
- Direct op een verse host uitvoerbaar (bijv. via `curl | sudo bash` na review)
...
- Interactieve profielkeuze (L1 Server / L2 Server) bij het starten
- Modulaire opbouw per CIS-sectie (1 t/m 7)
- Pre-flight check: OS-versie, root-rechten, netwerktoegang, schijfruimte
- Automatische back-up van gewijzigde configs naar `/var/backups/cis-hardening/<timestamp>/`
- Dry-run modus (`--dry-run`) die alleen rapporteert wat er gewijzigd zou worden
- Logging van elke actie naar `/var/log/cis-hardening.log`
- Rollback-script dat de laatste back-up terugzet
- Audit-only modus die compliance rapporteert zonder te wijzigen
- Kleurgecodeerde output (PASS / FAIL / SKIP / CHANGED)
...
├── README.md
├── LICENSE
├── harden.sh                  # Hoofd-entrypoint (interactieve menu)
...
```bash
git clone https://github.com/<jouw-user>/ubuntu-24.04-cis-hardening.git
...
- [ ] v0.1 – Projectstructuur, preflight-checks, logging-framework
...
Zie `docs/TESTING.md` voor details.
...
MIT License – zie `LICENSE`. CIS Benchmarks zijn eigendom van het Center for Internet Security; deze repo bevat geen CIS-benchmarkteksten, alleen implementaties van publiekelijk beschreven controls.

-----

## 🤝 Bijdragen

Pull requests zijn welkom. Open eerst een issue voor grotere wijzigingen. Zorg dat nieuwe controls de CIS-ID vermelden en een unit-test bevatten.
